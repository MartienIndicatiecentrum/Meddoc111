...
const NewTaskPage: React.FC = () => {
  ...
  const [uitvoerdatum, setUitvoerdatum] = useState<Date | undefined>(
    new Date()
  );
  const [deadlineDatum, setDeadlineDatum] = useState<Date | undefined>(
    undefined
  );
  const [showCalendar, setShowCalendar] = useState(false);
  const [showDeadlineCalendar, setShowDeadlineCalendar] = useState(false);
  const [huisbezoekDatum, setHuisbezoekDatum] = useState<Date | undefined>(
    undefined
  );
  ...
  const filteredOpdrachtgevers = opdrachtgevers.filter(o =>
    o.toLowerCase().includes(searchOpdr.toLowerCase())
  );
  const filteredSubopdrachtgevers = subopdrachtgevers.filter(s =>
    s.toLowerCase().includes(searchSub.toLowerCase())
  );
  const filteredTaakbeschrijvingen = taakbeschrijvingen.filter(t =>
    t.toLowerCase().includes(searchTaak.toLowerCase())
  );

  // Conditional rendering
  const showIndicatie =
    state.taakbeschrijving === 'Indicatie (nieuw/verlening/wijzigen)';
  const showVraagstelling =
    state.taakbeschrijving === 'Beantwoorden vragen verzekeraar';

  const validate = (): string | null => {
    if (!selectedClient) {
      return 'Selecteer eerst een cliënt.';
    }
    if (!state.opdrachtgever) {
      return 'Kies een opdrachtgever.';
    }
    if (state.subopdrachtgever === 'Overig' && !state.subopdrachtgeverOverig) {
      return 'Specificeer de subopdrachtgever.';
    }
    if (!state.taakbeschrijving) {
      return 'Kies een taakbeschrijving.';
    }
    if (
      state.taakbeschrijving === 'Indicatie (nieuw/verlening/wijzigen)' &&
      state.indicatieOpties.length === 0
    ) {
      return 'Selecteer minstens één indicatie-optie.';
    }
    if (
      state.taakbeschrijving === 'Beantwoorden vragen verzekeraar' &&
      state.vraagstellingOpties.length === 0
    ) {
      return 'Selecteer minstens één vraagstelling-optie.';
    }
    if (
      state.vraagstellingOpties.includes('Nieuwe vraagstelling') &&
      !state.vraagstellingDatum
    ) {
      ...
      try {
        ...
        if (!error && data?.path) {
          ...
          statusArr[i] = {
            success: false,
            message:
              error?.message || JSON.stringify(error) || 'Onbekende fout',
          };
        }
      } catch (err: any) {
        statusArr[i] = {
    ...
    <div className='bg-[#f8fafc] min-h-screen py-10 px-2 md:px-0'>
      <div className='max-w-7xl mx-auto'>
          ...
          {selectedClient && clientName && (
            <div className='p-3 bg-blue-50 border border-blue-200 rounded text-blue-900 font-medium md:ml-6 md:mt-0 mt-2'>
              <span className='inline-block px-2 py-0.5 rounded-full bg-purple-100 text-purple-600 border border-purple-500 font-medium text-xs'>
                Cliënt geselecteerd: {clientName}
            ...
            <div className='flex gap-4'>
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, doelStatus: 'nieuwe_taak' }))
                  }
              ...
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, doelStatus: 'openstaand' }))
                  }
              ...
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, doelStatus: 'lopende_zaak' }))
                  }
              ...
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, doelStatus: 'afgehandeld' }))
                  }
            ...
            <div className='flex gap-4'>
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, urgentieStatus: 'niet_urgent' }))
                  }
                  className='accent-blue-500 w-5 h-5'
              ...
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, urgentieStatus: 'urgent' }))
                  }
                  className='accent-orange-500 w-5 h-5'
                  aria-label='Urgent'
                />
              ...
              <label className='flex items-center gap-2 cursor-pointer'>
                <input
                  ...
                  onChange={() =>
                    setState(s => ({ ...s, urgentieStatus: 'zeer_urgent' }))
                  }
                  className='accent-red-500 w-5 h-5'
        ...
        {success ? (
          ...
          <form
            className='bg-white rounded-lg shadow p-6 md:p-10 space-y-6'
            autoComplete='off'
          ...
          >
            ...
            <div>
              <label className='block text-sm font-semibold mb-1'>
                Documenten uploaden (meerdere mogelijk)
              </label>
              <input
                type='file'
                ...
                onChange={e => {
                  ...
                  setUploadStatus(
                    selectedFiles.map(() => ({ success: false, message: '' }))
            ...
            <div>
              ...
              <select
                ...
                onChange={e =>
                  setState(s => ({ ...s, opdrachtgever: e.target.value }))
                }
                className='border rounded px-2 py-1 w-full'
              ...
              {showSubOverig && (
                <input
                  ...
                  onChange={e =>
                    setState(s => ({
                      ...s,
                      subopdrachtgeverOverig: e.target.value,
                    }))
                  }
            ...
            <div>
              ...
              <select
                ...
                onChange={e =>
                  setState(s => ({
                    ...s,
                    taakbeschrijving: e.target.value,
                    indicatieOpties: [],
                    vraagstellingOpties: [],
                  }))
                }
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setState(s => ({
                          ...s,
                          vraagstellingDatum: e.target.value,
                        }))
                      }
                      className='border rounded px-2 py-1 w-full'
            ...
            <div>
              ...
              <div className='flex items-center gap-4 flex-wrap'>
                ...
                >
                  {uitvoerdatum
                    ? format(uitvoerdatum, 'dd-MM-yyyy')
                    : 'Kies datum'}
                </Button>
                {showCalendar && (
            ...
            <div>
              ...
              <div className='flex items-center gap-4 flex-wrap'>
                ...
                >
                  {deadlineDatum
                    ? format(deadlineDatum, 'dd-MM-yyyy')
                    : 'Kies datum'}
                </Button>
            ...
            <div>
              ...
              <div className='flex items-center gap-4 flex-wrap'>
                ...
                >
                  {huisbezoekDatum
                    ? format(huisbezoekDatum, 'dd-MM-yyyy')
                    : 'Kies datum'}
                </Button>
