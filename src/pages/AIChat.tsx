...
import type { Message, DocumentSource as DocumentSourceType, ChatResponse, Document, PromptTemplate } from '@/types/chat';
import {
  Send,
  Bot,
  User,
...
const AIChat: React.FC = () => {
  ...
  const [isChatHistoryDropdownOpen, setIsChatHistoryDropdownOpen] = useState(false);
  ...
  const [selectedClientName, setSelectedClientName] = useState<string>('Alle cliënten');
  ...
  useEffect(() => {
    ...
    serviceManager.waitForServices().then((status) => {
      ...
      try {
        ...
        if (savedChat && savedChat.messages.length > 0) {
          ...
          const welcomeMessage: Message = {
            id: '1',
            type: 'assistant',
            content: 'Welkom! 👋 Ik ben uw AI-assistent voor medische documenten. U kunt mij vragen stellen in gewoon Nederlands - bijvoorbeeld "Wat zijn de medicijnen van mevrouw Jansen?" of "Wanneer verlopen de indicaties?". Hoe kan ik u helpen?',
            timestamp: new Date(),
            status: 'sent'
          };
  ...
  }, [messages, currentChatId, documentSource, selectedClientId, selectedDocument, isLoadingChat]);
  ...
  const checkMorphikAvailability = async () => {
    try {
      ...
      setMorphikStatus({ available: false, error: {
        type: 'network',
  ...
  const sendMessageToRAG = async (question: string): Promise<ChatResponse> => {
    try {
      ...
      const data = await response.json();
      return {
        answer: data.answer || 'I couldn\'t find relevant information for your question.',
  ...
  const sendMessageToDatabase = async (question: string): Promise<ChatResponse> => {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          if (data.error?.includes('SUPABASE_URL')) {
            ...
            errorMessage += 'Cannot connect to the database. Please check your Supabase configuration.';
          } else {
      ...
      return {
        answer: data.response || 'I couldn\'t find relevant information in the database.',
        sources: data.sources || []
      };
    } catch (error) {
  ...
  const sendMessageToMorphik = async (question: string): Promise<ChatResponse> => {
    try {
      ...
      return {
        answer: 'Er is een fout opgetreden bij het communiceren met Morphik AI. Controleer of de service beschikbaar is.',
        error: error instanceof Error ? error.message : 'Unknown error'
      };
    }
  };
    ...
    if (showClientSuggestions) {
      if (e.key === 'ArrowDown') {
        ...
        if (filteredClientNames[selectedSuggestionIndex]) {
          ...
          const newMessage = inputMessage.substring(0, cursorPosition) + filteredClientNames[selectedSuggestionIndex];
  ...
  const startNewChat = () => {
    ...
    let welcomeMessage = '';
    if (documentSource === 'uploaded') {
      welcomeMessage = 'Welkom terug! 👋 Ik ben klaar om uw vragen over de geüploade documenten te beantwoorden. Stel gerust uw vraag in gewoon Nederlands.';
    } else if (documentSource === 'database') {
      welcomeMessage = 'Welkom terug! 👋 Ik ben klaar om uw vragen over de medische database te beantwoorden. Vraag bijvoorbeeld naar cliënten, documenten of taken.';
    } else {
  ...
  const toggleDocumentSource = () => {
    setDocumentSource(prev => {
      if (prev === 'uploaded') return 'database';
      if (prev === 'database') return 'morphik';
      return 'uploaded';
    });
  ...
  const getSmartPromptTemplates = (): PromptTemplate[] => {
    if (documentSource === 'uploaded') {
      ...
      const baseTemplates: PromptTemplate[] = [
        ...
        {
          label: "Cliënt info",
          prompt: "Wat zijn de belangrijkste gegevens van cliënt [naam] in dit document?",
          icon: "👤",
          hasPlaceholder: true
        },
        {
          label: "Medicatie",
          prompt: "Welke medicatie wordt voorgeschreven en wat zijn de doseringen?",
          icon: "💊",
          hasPlaceholder: false
        },
        {
      ...
      if (selectedDocument) {
        const docTitle = documents.find(d => d.id === selectedDocument)?.title || '';
        if (docTitle.toLowerCase().includes('indicatie')) {
          baseTemplates.splice(2, 0, {
            label: "Indicatie details",
            prompt: "Wat zijn de details van de zorgindicatie (uren, looptijd, functies)?",
            icon: "📊",
            hasPlaceholder: false
          });
          ...
          baseTemplates.splice(2, 0, {
            label: "PGB budget",
            prompt: "Wat is het PGB budget en welke voorwaarden zijn van toepassing?",
            icon: "💰",
            hasPlaceholder: false
          });
        }
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <Alert className="mb-6 border-red-200 bg-red-50">
          ...
          <AlertTitle className="text-red-800">Services Not Available</AlertTitle>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="border-l-4 border-blue-500 pl-4">
                <h3 className="font-medium text-lg mb-2">1. Check Environment Setup</h3>
                <p className="text-gray-600 mb-3">First, verify your environment is configured:</p>
                <div className="bg-gray-100 rounded p-3 flex items-center justify-between">
                  <code className="text-sm">npm run check:env</code>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => copyCommand('npm run check:env', 'check-env')}
                  >
              ...
              <div className="border-l-4 border-blue-500 pl-4">
                ...
                <p className="text-gray-600 mb-3">If you haven't already, create your .env file:</p>
                <div className="space-y-2">
                  <div className="bg-gray-100 rounded p-3 flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => copyCommand('cp .env.example .env', 'copy-env')}
                    >
              ...
              <div className="border-l-4 border-green-500 pl-4">
                ...
                <p className="text-gray-600 mb-3">Run all required services with one command:</p>
                <div className="bg-gray-100 rounded p-3 flex items-center justify-between">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${servicesAvailable.backend ? 'bg-green-500' : 'bg-red-500'}`} />
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
                ...
                {!servicesAvailable.morphik && morphikStatus.error && (
                  <div className="text-right">
                    <span className="text-sm text-red-600 block">{morphikStatus.error.message}</span>
                    {morphikStatus.error.troubleshooting && (
                      <span className="text-xs text-gray-500 block mt-1">{morphikStatus.error.troubleshooting}</span>
                    )}
                  </div>
                )}
                {!servicesAvailable.morphik && !morphikStatus.error && (
                  <span className="text-sm text-red-600">Service niet beschikbaar</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 rounded-lg p-6">
            ...
            <ul className="space-y-2 text-sm text-blue-800">
              <li>• Run <code className="bg-blue-100 px-1 rounded">npm run verify</code> for detailed diagnostics</li>
              <li>• Check the console where you ran npm run dev for error messages</li>
              <li>• Make sure ports 3000, 5001, and 8081 are not in use</li>
              <li>• See AI_CHAT_TROUBLESHOOTING.md for detailed troubleshooting</li>
            </ul>
          </div>
          ...
          {!servicesAvailable.morphik && (
            <div className="bg-red-50 rounded-lg p-6">
              <h3 className="font-medium text-lg mb-2 text-red-900">Morphik AI Troubleshooting</h3>
              <div className="space-y-3 text-sm text-red-800">
                {morphikStatus.error?.type === 'config' && (
                  <div>
                    <p className="font-medium">Configuratie probleem:</p>
                    <ul className="list-disc list-inside space-y-1 mt-1">
                      <li>Controleer of <code className="bg-red-100 px-1 rounded">VITE_MORPHIK_API_KEY</code> is ingesteld in uw .env bestand</li>
                      <li>Controleer of <code className="bg-red-100 px-1 rounded">VITE_MORPHIK_API_URL</code> correct is geconfigureerd</li>
                ...
                {morphikStatus.error?.type === 'auth' && (
                  <div>
                    ...
                    <ul className="list-disc list-inside space-y-1 mt-1">
                      <li>Controleer of uw Morphik AI API-sleutel nog geldig is</li>
                      <li>Vraag een nieuwe API-sleutel aan bij Morphik AI</li>
                      <li>Controleer of uw API-sleutel de juiste permissies heeft</li>
                    </ul>
                  </div>
                )}
                ...
                {morphikStatus.error?.type === 'api' && (
                  <div>
                    ...
                    <ul className="list-disc list-inside space-y-1 mt-1">
                      ...
                      <li>Contacteer Morphik AI ondersteuning als het probleem aanhoudt</li>
                    </ul>
                  </div>
                )}
          ...
          <div className="flex-shrink-0 p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h2 className="text-xl font-semibold">Hoe werkt de AI-assistent?</h2>
              </div>
              <button
                onClick={() => setShowHelpDialog(false)}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600 mb-3">
                  U hoeft geen speciale commando's te gebruiken. Stel uw vragen zoals u dat normaal zou doen:
                </p>
                <div className="bg-blue-50 rounded-lg p-4 space-y-2">
                  <div className="font-medium text-blue-900">Voorbeelden:</div>
                  <ul className="space-y-1 text-sm text-blue-800">
                    <li>• "Wat zijn de medicijnen van [cliëntnaam]?"</li>
                    <li>• "Wanneer moet de indicatie van [cliëntnaam] verlengd worden?"</li>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="border border-blue-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-600">
                      Zoek in specifieke PDF's die u heeft geüpload. Perfect voor het analyseren van individuele documenten.
                    </p>
                  </div>
                  <div className="border border-green-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-600">
                      Zoek door alle documenten in het systeem. Ideaal voor overzichten en het vinden van patronen.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start space-x-2">
                    ...
                    <span>Wees specifiek: "medicatie van mevrouw Jansen" werkt beter dan "medicatie"</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                  ...
                  <li className="flex items-start space-x-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                    <span>De snelle prompts zijn aanpasbaar - klik en wijzig ze naar behoefte</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <span className="text-green-500 mt-0.5">✓</span>
                    <span>Bij database zoekopdrachten kunt u ook tijdsperiodes opgeven</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <div className="flex flex-col h-full bg-gray-50">
          ...
          <div className="bg-white border-b border-gray-200 px-6 py-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">AI Document Assistant - Setup Required</h1>
                <p className="text-sm text-gray-600">Backend services need to be started</p>
              </div>
            </div>
          </div>
      ...
      <TooltipProvider>
        ...
        <div className="flex flex-col h-full bg-gray-50">
          ...
          <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Bot className={`w-8 h-8 text-blue-600 ${!hasSeenWelcome ? 'animate-bounce' : ''}`} />
                <div>
                  ...
                  <TooltipTrigger asChild>
                    <button
                      onClick={() => {
                        ...
                        localStorage.setItem('ai-chat-sound-enabled', (!soundEnabled).toString());
                        toast.success(soundEnabled ? 'Geluiden uitgeschakeld' : 'Geluiden ingeschakeld');
                      }}
              ...
              <div className="flex items-center space-x-2 bg-gray-100 rounded-lg p-2">
                <div className={`flex items-center space-x-2 px-2 py-1 rounded-md transition-colors cursor-pointer ${
                  documentSource === 'uploaded' ? 'bg-blue-500 text-white' : 'text-gray-600 hover:bg-gray-200'
              ...
              ) : (documentSource === 'morphik' && !servicesAvailable.morphik) ? (
                <div className="flex items-center space-x-2 px-3 py-1 bg-red-100 text-red-700 rounded-lg" title={morphikStatus.error?.troubleshooting}>
                  <span className="text-xs font-medium">{morphikStatus.error?.message || 'Morphik AI Offline'}</span>
                </div>
              ) : null}
                ...
                {recentChats.length > 0 && (
                  <div className="relative">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          onClick={() => setIsChatHistoryDropdownOpen(!isChatHistoryDropdownOpen)}
                          className="flex items-center space-x-2 px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
                        >
                          <MessageCircle className="w-4 h-4" />
                          <span className="text-sm">Gesprekken</span>
                          <span className="text-xs bg-gray-300 px-2 py-0.5 rounded-full">{recentChats.length}</span>
                    ...
                    {isChatHistoryDropdownOpen && (
                      <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-96 overflow-y-auto">
                        <div className="p-3 border-b border-gray-200">
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm font-medium text-gray-900">Recente gesprekken</h3>
                            <button
                              onClick={() => {
                                clearAllChatHistory();
                                setRecentChats([]);
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  <div className={`w-2 h-2 rounded-full ${
                                    chat.documentSource === 'uploaded' ? 'bg-blue-500' :
                                    chat.documentSource === 'database' ? 'bg-green-500' : 'bg-purple-500'
                                  }`} />
                ...
                <div className="flex-1 max-w-md">
                  <KiesClientDropdown 
                    ...
                    onClientSelect={(clientId, clientName) => {
                      console.log('Client selected with name:', clientId, clientName);
                      setSelectedClientId(clientId || null);
          ...
          {documentSource === 'uploaded' && (
            <div className="w-80 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
              <div className="p-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900 mb-3">Document Context</h2>
                  ...
                  <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded-md">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-green-800 font-medium">
                        Context: {documents.find(d => d.id === selectedDocument)?.title}
                      </span>
                      <button
                  ...
                  {getSuggestedQuestions().map((question, index) => (
                    ...
                    >
                      <div className="flex items-start space-x-2">
                        <span className="text-blue-400 group-hover:text-blue-600 mt-0.5">→</span>
                        <span>{question}</span>
                      </div>
                    </button>
                  ))}
              ...
              <div className="flex-1 p-4 overflow-y-auto">
                ...
                <div className="space-y-2">
                  {getSuggestedQuestions().map((question, index) => (
                    ...
                    >
                      <div className="flex items-start space-x-2">
                        <span className="text-green-400 group-hover:text-green-600 mt-0.5">→</span>
                        <span>{question}</span>
                      </div>
                    </button>
                  ))}
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-200 animate-in fade-in duration-500">
                            ...
                            <div className="space-y-2">
                              {message.sources.map((source, index) => {
                                const isDetailedSource = typeof source === 'object';
                                return (
                                    ...
                                    {isDetailedSource ? (
                                      <div className="flex-1 text-xs">
                                        ...
                                        <div className="text-gray-500 mt-0.5">
                                          ...
                                          {source.client_name && source.client_name !== 'Onbekende cliënt' && (
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {message.documentSource && (
                              <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">
                                {message.documentSource === 'database' ? '🗄️ Database' : '📁 Uploaded'}
                              </span>
                            )}
                            {message.type === 'user' && message.status && (
                              <span className="flex items-center space-x-1">
                                {message.status === 'sending' && <Check className="w-3 h-3 text-gray-400" />}
                                {message.status === 'sent' && <CheckCheck className="w-3 h-3 text-blue-600" />}
                ...
                <div className="flex justify-start animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="max-w-3xl flex space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center animate-pulse ${
                      documentSource === 'database' ? 'bg-green-600 text-white' : 'bg-gray-600 text-white'
                    }`}>
                      {documentSource === 'database' ? <Database className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                    </div>
                    <div className="bg-white border border-gray-200 rounded-lg px-4 py-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-sm text-gray-600">
                          {documentSource === 'database' ? 'Database doorzoeken...' : 'Documenten analyseren...'}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex-shrink-0 border-t border-gray-200 bg-white">
              ...
              <div className="border-b border-gray-100 px-4 py-3">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-gray-700">Snelle prompts - klik om te gebruiken:</span>
                  <Tooltip>
                    <TooltipTrigger>
                    ...
                    {getSmartPromptTemplates().map((template, index) => (
                      ...
                      >
                        <span className="flex items-center space-x-1.5">
                          <span className="text-base transition-transform group-hover:scale-110">{template.icon}</span>
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      onKeyDown={(e) => {
                        ...
                        if (showClientSuggestions && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {
                          handleKeyPress(e);
                        }
                      }}
                    ...
                    {showClientSuggestions && (
                      <div className="absolute bottom-full mb-2 left-0 w-full max-w-md bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                        <div className="p-2">
                          <div className="text-xs text-gray-500 mb-1 px-2">Selecteer een cliënt:</div>
