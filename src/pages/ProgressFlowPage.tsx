        ...
        >
          ...
          <div className='flex-1 flex flex-col gap-3 min-h-0 overflow-y-auto'>
            {items.length === 0 ? (
              ...
              items.map((item, idx) => (
                <ProgressItemCard key={item.id} item={item} index={idx} />
              ))
            )}
...
const ProgressFlowPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    const inProgressItems = progressItems.filter(
      item => item.stage === 'in_progress'
    ).length;
    const onHoldItems = progressItems.filter(
      item => item.stage === 'on_hold'
    ).length;
  ...
  const onDragEnd = useCallback(result => {
    ...
    toast.success(
      `Item verplaatst naar ${PROGRESS_STAGES.find(s => s.key === newStage)?.label}`
    );
  }, []);

  const handleAddItem = useCallback(stage => {
    toast.info(
      `Functionaliteit voor toevoegen van nieuwe items in ${stage} komt binnenkort`
    );
  }, []);
      ...
      <div className='max-w-full mx-auto py-8 px-4 sm:px-6 lg:px-8'>
          ...
          <div className='flex justify-between items-start mb-6'>
            <div>
              ...
              <p className='text-gray-600'>
                Volg de voortgang van projecten en processen via een visueel
                kanban bord
              </p>
            </div>
        ...
        <div className='mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4'>
          ...
          <ul className='text-sm text-blue-800 space-y-1'>
            ...
            <li>
              • Items worden automatisch gesorteerd op prioriteit en deadline
            </li>
          </ul>
        </div>
      </div>
