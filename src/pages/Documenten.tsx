...
const fetchCategories = async (): Promise<string[]> => {
  ...
  const categories = Array.from(
    new Set(data?.map(doc => doc.category).filter(Boolean) || [])
  ) as string[];
  return categories;
};
...
const Documenten: React.FC = () => {
    ...
    if (selectedFolder) {
      if (selectedFolder === 'ALL') {
        ...
        const filtered = documents.filter(doc => {
          ...
          const matchesClient =
            !selectedClient || String(doc.client_id) === String(selectedClient);
      ...
      if (selectedFolder === 'UNKNOWN') {
        ...
        const filtered = documents.filter(doc => {
          if (doc.type === 'folder') {
            return false;
          } // Don't show folder items themselves

          const isUnknownFolder =
            !doc.category || !folderNames.includes(doc.category);
          const matchesSearch =
            doc.title.toLowerCase().includes(search.toLowerCase()) ||
            (doc.description &&
              doc.description.toLowerCase().includes(search.toLowerCase()));
          ...
          return (
            isUnknownFolder && matchesSearch && matchesCategory && matchesClient
          );
        });
      ...
      const selectedFolderData = folders.find(
        f => String(f.id) === String(selectedFolder)
      );
  ...
  const handleFileUpload = async (
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        ...
        console.log(
          'Uploading to Supabase Storage with filename:',
          safeFileName
        );
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from('documents').getPublicUrl(safeFileName);

        console.log('Public URL obtained:', publicUrl);

        // Find client name if client is selected
        const clientName = selectedClient
          ? clients.find(c => String(c.id) === String(selectedClient))?.naam ||
            'Onbekende cliënt'
          : null;
  ...
  const handleBulkDownload = () => {
    if (selectedDocuments.length > 0) {
      toast.success(
        `${selectedDocuments.length} documenten worden gedownload...`
      );
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const itemsToDelete = documents.filter(doc =>
        selectedDocuments.includes(doc.id)
      );
      console.log('Items found to delete:', itemsToDelete);

      // Separate documents and folders
      const documentsToDelete = itemsToDelete.filter(
        item => item.type !== 'folder'
      );
      const foldersToDelete = itemsToDelete.filter(
        item => item.type === 'folder'
      );
      ...
      if (foldersToDelete.length > 0) {
        ...
        if (resetError) {
          console.warn(
            'Could not reset documents from deleted folders:',
            resetError
          );
        }
      }
      ...
      toast.error(
        error instanceof Error ? error.message : 'Kon items niet verwijderen'
      );
    }
  };
  ...
  const moveClientDocumentsToFolder = async (
    clientId: string,
    folderName: string
  ) => {
    try {
      ...
      const documentsToMove = documents.filter(
        doc =>
          String(doc.client_id) === String(clientId) && doc.type !== 'folder'
      );
      ...
      if (documentsToMove.length > 0) {
        ...
        documentsToMoveFinal = availableDocuments.slice(
          0,
          Math.min(3, availableDocuments.length)
        );
      }
      ...
      const clientName =
        clients.find(c => String(c.id) === String(clientId))?.naam ||
        'Onbekende cliënt';
      toast.success(
        `${documentsToMoveFinal.length} documenten zijn verplaatst naar map "${folderName}" voor ${clientName}`
      );
  ...
  return (
    <AppLayout>
      <div className='flex gap-4 items-center mb-6 ml-2 mt-4'>
        <Link
          to='/'
          className='flex items-center px-3 py-1.5 rounded-full border border-black hover:bg-gray-100 transition shadow-sm'
        >
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='flex items-center gap-2'>
                  <select
                    value={sortBy}
                    onChange={e =>
                      setSortBy(e.target.value as 'date' | 'name' | 'type')
                    }
            ...
            <div className='mt-4 pt-4 border-t border-gray-200'>
              <div className='flex items-center justify-between text-sm text-gray-600'>
                <div className='flex items-center gap-4'>
                  <span>
                    Totaal: <strong>{filteredDocuments.length}</strong>{' '}
                    documenten
                  </span>
                  <span>
                    Geselecteerd: <strong>{selectedDocuments.length}</strong>
                  </span>
                </div>
                <div className='flex items-center gap-2'>
                  <button
                              ...
                              <div className='flex items-center gap-2 ml-4'>
                                <button
                                  onClick={e => {
                                    e.stopPropagation();
                                    console.log(
                                      '🔍 Bekijk button clicked, document:',
                                      doc
                                    );
                                    console.log(
                                      '🔍 Document has file_path:',
                                      doc.file_path
                                    );
                                    setSelectedDocument(doc);
                                  }}
                          ...
                          <div className='flex gap-1'>
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                console.log(
                                  '🔍 Grid bekijk button clicked, document:',
                                  doc
                                );
                                console.log(
                                  '🔍 Grid document has file_path:',
                                  doc.file_path
                                );
                                setSelectedDocument(doc);
                              }}
                    ...
                    {moveClientDocuments && (
                      <div>
                        ...
                        <select
                          ...
                          onChange={e =>
                            setSelectedClientForFolder(e.target.value)
                          }
                        ...
                        >
                          ...
                          {clients.map(client => {
                            const clientDocCount = documents.filter(
                              doc =>
                          ...
                          <p className='text-xs text-blue-600 mt-2'>
                            {
                              documents.filter(
                                doc =>
                                  doc.client_id ===
                                    parseInt(selectedClientForFolder) &&
                                  doc.type !== 'folder'
                              ).length
                            }{' '}
                            documenten worden verplaatst naar de nieuwe map.
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <div className='flex justify-end gap-3'>
                    ...
                    <button
                      ...
                      disabled={
                        isCreatingFolder ||
                        !newFolderName.trim() ||
                        (moveClientDocuments && !selectedClientForFolder)
                      }
