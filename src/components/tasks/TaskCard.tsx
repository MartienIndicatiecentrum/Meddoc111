...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <div
      className={`flex flex-col md:flex-row items-start md:items-center gap-2 border-l-4 p-4 shadow-sm rounded-md mb-2 ${priorityColor[task.priority]}`}
    >
      {/* Status indicator */}
      <span
        className={`px-2 py-1 rounded text-xs font-semibold ${statusColor[task.status]}`}
      >
        {task.status}
      </span>
      {/* Title & type */}
      ...
      {/* Priority badge */}
      <span
        className={`ml-2 px-2 py-1 rounded text-xs font-bold uppercase ${priorityColor[task.priority]}`}
      >
        {task.priority}
      </span>
      {/* Quick actions */}
