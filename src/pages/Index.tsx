...
const Index = () => {
  ...
  const filteredDocuments = recentDocuments.filter(doc => {
    ...
    const statusOk =
      ...
      (statusFilter === 'openstaand' &&
        (doc.status === 'openstaand' || doc.status === 'nieuw'));
    const typeOk =
      typeFilter === 'alle' || (doc.category && doc.category === typeFilter);
    const urgentieOk =
      urgentieFilter === 'alle' || doc.urgency === urgentieFilter;
  ...
  const fetchRecentDocuments = useCallback(async () => {
    try {
      ...
      toast({
        title: 'Fout',
        description:
          'Er is een fout opgetreden bij het ophalen van recente documenten.',
        variant: 'destructive',
      });
    }
  }, [selectedClientId]);
  ...
  const fetchDocumentStats = useCallback(async () => {
    try {
      let query = supabase.from('documents').select('id, status, created_at');
      if (selectedClientId) {
        query = query.eq('client_id', selectedClientId);
      }
      const { data, error } = await query;
      ...
      if (data) {
        const newStats = data.reduce(
          (acc: DocumentStats, doc: Document) => {
            acc.total++;
            switch (doc.status) {
              case 'afgehandeld':
                acc.afgehandeld++;
      ...
      toast({
        title: 'Fout',
        description:
          'Er is een fout opgetreden bij het ophalen van de statistieken.',
        variant: 'destructive',
      });
    }
  }, [selectedClientId]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        await Promise.all([fetchDocumentStats(), fetchRecentDocuments()]);
      } catch (error) {
    ...
    <div className='flex flex-col min-h-screen'>
      ...
      {selectedClientId && (
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 flex flex-wrap gap-4 justify-center mb-8'>
          <Button
            onClick={() => navigate('/documenten-inzien')}
            variant='outline'
          >
            Documenten inzien
          </Button>
          <Button onClick={() => navigate('/documenten')} variant='default'>
            Nieuw document toevoegen
          </Button>
          <Button onClick={() => navigate('/nieuwe-taak')} variant='default'>
            Nieuwe taak
          </Button>
          <Button onClick={() => navigate('/taken')} variant='default'>
            Takenoverzicht
          </Button>
          <Button onClick={() => setShowOpenTasks(true)} variant='outline'>
            Openstaande taken inzien
          </Button>
          <Button onClick={() => setShowClosedTasks(true)} variant='outline'>
