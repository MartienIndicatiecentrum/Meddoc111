...
const SUPABASE_SERVICE_ROLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx0YXNqYmdhbW9sanZxb2NsZ2tmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MjQwODU1MywiZXhwIjoyMDY3OTg0NTUzfQ.PdVN2_rq9m_EbR8jNJKbzImPz5rVip0i18iBrGEXgps";
...
(async () => {
  ...
  for (const doc of docs) {
    ...
    // Split bucket en pad
    let [bucket, ...pathParts] = filePath.replace(/^\/+/, '').split('/');
    ...
    try {
      ...
      if (err.response) {
        console.log(`❌ Fout bij document ${doc.id}: ${err.response.status} - ${err.response.statusText}`);
      } else {
