...
const DocumentList: React.FC<DocumentListProps> = ({ clientId, onDocumentClick, filterStatus }) => {
  ...
  useEffect(() => {
    ...
    subscription = supabase.channel('public:documents')
        ...
        <tbody>
          {documents
            .filter(doc => {
              ...
              if (filterStatus === 'PRIORITEIT_URGENT') return doc.priority === 'urgent';
            ...
            .map((doc) => (
              <tr key={doc.id} className="hover:bg-gray-50">
                ...
                <td className="py-2 px-4 border-b">
                  {doc.status ? (
                    <span className={`inline-block rounded px-2 py-1 text-xs font-semibold ${statusColors[doc.status] || 'bg-gray-100 text-gray-800'}`}>{doc.status.replace('_', ' ')}</span>
                  ) : (
                    <span className="text-gray-400">-</span>
                  )}
                </td>
                <td className="py-2 px-4 border-b">
                  {doc.created_at ? new Date(doc.created_at).toLocaleDateString('nl-NL') : <span className="text-gray-400">-</span>}
                </td>
              </tr>
            ))}
        </tbody>
