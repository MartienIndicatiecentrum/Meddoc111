...
const CLIENT_FIELDS = [
  ...
  {
    key: 'polisnummer',
    labels: ['polisnummer', 'polis nummer', 'polis', 'verzekeringsnummer'],
  },
  {
    key: 'telefoon',
    labels: ['telefoon', 'telefoonnummer', 'tel', 'mobiel', 'nummer'],
  },
  { key: 'postcode', labels: ['postcode', 'post code', 'zip'] },
  {
    key: 'woonplaats',
    labels: ['woonplaats', 'woon plaats', 'plaats', 'stad', 'gemeente'],
  },
  { key: 'huisarts', labels: ['huisarts', 'huis arts', 'dokter', 'arts'] },
  {
    key: 'notities',
    labels: ['notities', 'notitie', 'aantekeningen', 'opmerkingen'],
  },
];
...
const ChatWindow = () => {
  ...
  function detectClientField(text: string) {
    ...
    const cleanText = lower
      .replace(/wat is|wat zijn|wat was|welke|wanneer|wie is|hoe heet/g, '')
      .trim();

    console.log(
      '[ChatWindow] Detecting field for query:',
      text,
      'Clean text:',
      cleanText
    );
  ...
  async function handleSend() {
    ...
    if (isDbQuery(input)) {
      ...
      if (input.toLowerCase().includes('document')) {
        const { data, error } = await supabase
          ...
          .limit(5);
        if (error) {
          dbResult = `Fout: ${error.message}`;
        } else if (data && data.length > 0) {
          dbResult = (data as Document[])
            .map(d => `${d.title || d.naam || d.id}`)
            .join('\n');
        } else {
          dbResult = 'Geen documenten gevonden.';
        }
        ...
        if (error) {
          ...
          dbResult = 'Geen taken gevonden.';
        }
      } else {
        dbResult =
          "Onbekende database-actie. Probeer: 'Toon documenten' of 'Toon taken'.";
      }
    ...
    if (field && clientId) {
      console.log(
        '[ChatWindow] Fetching client field:',
        field,
        'for client:',
        clientId
      );
      ...
      try {
        ...
        if (error) {
          ...
          setMessages(msgs => [
            ...
            {
              sender: 'db',
              text: `Fout bij ophalen gegevens: ${error.message}`,
            },
          ]);
        } else if (!data) {
          setMessages(msgs => [
          ...
          setMessages(msgs => [
            ...
            {
              sender: 'db',
              text: `${field} van ${clientName}: ${displayValue}`,
            },
          ]);
        }
      } catch (err) {
    ...
    console.log(
      '[ChatWindow] Sending to AI endpoint with client context:',
      clientData ? 'Yes' : 'No'
    );
    ...
    try {
      ...
      setMessages(msgs => [
        ...
        {
          sender: 'ai',
          text: 'Er is een fout opgetreden bij het verwerken van je vraag. Probeer het opnieuw.',
        },
      ]);
    }
