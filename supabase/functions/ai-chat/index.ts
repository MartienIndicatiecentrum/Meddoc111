...
serve(async (req) => {
  ...
  try {
    ...
    const { query, documentId, userId, clientId, clientData, context: additionalContext } = await req.json();

    console.log('AI Chat request:', { query, documentId, userId, clientId, hasClientData: !!clientData });
    ...
    if (similarDocs && similarDocs.length > 0) {
      documentContext = '## Relevante documenten:\n\n' + similarDocs.map((doc: any) => {
        const preview = doc.content ? doc.content.substring(0, 500) + '...' : 'Geen inhoud beschikbaar';
    ...
    if (clientId && (!similarDocs || similarDocs.length === 0)) {
      console.log('No client-specific documents found, searching all documents...');
