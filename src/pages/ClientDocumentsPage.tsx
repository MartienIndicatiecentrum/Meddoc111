...
const ClientDocumentsPage: React.FC<ClientDocumentsPageProps> = ({
  clientId,
}) => {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      const { data, error } = await supabase
        .from('documents')
        .select(
          'id, title, type, category, status, created_at, file_path, file_size, client_id, date'
        )
  ...
  if (!clientId) {
    return <div className='p-8'>Geen cliënt geselecteerd.</div>;
  }
  if (loading) {
              ...
              <tr key={doc.id}>
                ...
                <td className='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
                  <Button
                    ...
                    onClick={() => {
                      console.log(
                        '🔍 Bekijk button clicked in ClientDocumentsPage, document:',
                        doc
                      );
