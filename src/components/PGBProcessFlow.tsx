...
const STAP_DEFINITIES = [
  ...
  {
    naam: 'indicatie_opstellen',
    label: 'Indicatie\nopstellen',
    volgorde: 3,
    icon: FileText,
  },
  {
    naam: 'indicatie_opsturen',
    label: 'Indicatie\nopsturen',
    volgorde: 4,
    icon: Building,
  },
  {
    naam: 'aanvullende_vragen_open',
    label: 'Aanvullende vragen verzekeraar openstaand',
    volgorde: 5,
    icon: MessageSquare,
  },
  {
    naam: 'aanvullende_vragen_afgerond',
    label: 'Aanvullende vragen verzekeraar afgerond',
    volgorde: 6,
    icon: CheckCircle,
  },
];
...
export default function PGBProcessFlow({ clientId }) {
  ...
  useEffect(() => {
    ...
    async function fetchProcesStappen() {
      ...
      const stappen = STAP_DEFINITIES.map(stap => {
        ...
        return dbStap
          ? { ...dbStap, label: stap.label, icon: stap.icon }
          : {
              code: stap.naam,
  ...
  useEffect(() => {
    ...
    async function fetchOpmerkingen() {
      ...
      if (data) {
        const map = {};
        data.forEach(row => {
          if (!map[row.stap_code]) {
            map[row.stap_code] = [];
          }
          map[row.stap_code].push({
            opmerking: row.opmerking,
            opmerking_datum: row.opmerking_datum,
          });
        });
        Object.keys(map).forEach(code => {
          map[code].sort((a, b) =>
            (b.opmerking_datum || '').localeCompare(a.opmerking_datum || '')
          );
        });
        setOpmerkingen(map);
      } else {
  ...
  const handleStatusChange = async (stapCode, nieuweStatus) => {
    ...
    setOptimisticStappen(prev =>
      prev
        ? prev.map(s =>
            s.code === stapCode ? { ...s, status: nieuweStatus } : s
          )
        : null
    );
  ...
  const updateSingleStap = async (
    ...
    try {
      const afgehandeldOp =
        nieuweStatus === 'afgehandeld'
          ? customAfgehandeldOp || new Date().toISOString()
          : null;

      const { error } = await supabase.from('proces_stappen').upsert({
        client_id: clientId,
        code: stapCode,
        status: nieuweStatus,
      ...
      if (error) {
        ...
        toast({
          title: 'Fout',
          description:
            'Er is een fout opgetreden bij het bijwerken van de stap.',
          variant: 'destructive',
        });
        return;
      }
      ...
      setProcesStappen(prev =>
        prev.map(s =>
          s.code === stapCode
            ? { ...s, status: nieuweStatus, afgehandeld_op: afgehandeldOp }
            : s
        )
      );
  ...
  const handleOpmerkingSave = async stapCode => {
    ...
    try {
      const { error } = await supabase.from('stap_opmerkingen').insert({
        client_id: clientId,
        stap_code: stapCode,
        opmerking: opmerking,
      ...
      if (error) {
        ...
        toast({
          title: 'Fout',
          description:
            'Er is een fout opgetreden bij het opslaan van de opmerking.',
          variant: 'destructive',
        });
        return;
      }
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          title: 'Fout',
          description:
            'Er is een fout opgetreden bij het bijwerken van de opmerking.',
          variant: 'destructive',
        });
        return;
      }
      ...
      setEditingOpmerking(prev => ({
        ...prev,
        [`${stapCode}_${index}`]: false,
      }));
  ...
  const toggleUrgent = stapCode => {
    setUrgentStappen(prev => {
      ...
      localStorage.setItem(
        `${URGENT_KEY}_${clientId}`,
        JSON.stringify(newUrgent)
      );
      return newUrgent;
    });
  };
    ...
    <div className='flex items-center gap-2'>
      <Switch
        ...
        onChange={e =>
          handleStatusChange(
            stap.code,
            e.target.checked ? 'afgehandeld' : 'openstaand'
          )
        }
        color='primary'
        size='small'
  ...
  const renderVoegOpmerkingToeKnop = (stap, idx) => (
    <button
      ...
      onClick={() =>
        setOpmerkingOpen(prev => ({ ...prev, [stap.code]: !prev[stap.code] }))
      }
    >
        ...
        <CardHeader>
          <div className='text-center'>
            ...
            <p className='text-gray-600'>
              Beheer de voortgang van het PGB proces
            </p>
          </div>
        </CardHeader>
        <CardContent>
                ...
                >
                  <CardContent className='p-4'>
                    ...
                    {isCompleted && stap.afgehandeld_op && (
                      <div className='mb-4 p-2 bg-green-100 rounded-lg'>
                        <div className='flex items-center gap-2 text-xs text-green-700'>
                          ...
                          <span>
                            Afgehandeld:{' '}
                            {new Date(stap.afgehandeld_op).toLocaleDateString(
                              'nl-NL'
                            )}
                          </span>
                        </div>
                    ...
                    <div className='space-y-3'>
                      ...
                      <button
                        onClick={() =>
                          setOpmerkingOpen(prev => ({
                            ...prev,
                            [stap.code]: !prev[stap.code],
                          }))
                        }
                        ...
                        <div className='space-y-3'>
                          ...
                          <div className='space-y-2'>
                            <textarea
                              ...
                              onChange={e =>
                                setNieuweOpmerking(prev => ({
                                  ...prev,
                                  [stap.code]: e.target.value,
                                }))
                              }
                          ...
                          {/* Existing Comments */}
                          {opmerkingen[stap.code] &&
                            opmerkingen[stap.code].length > 0 && (
                              <div className='space-y-2'>
                                <Separator />
                                <h4 className='text-xs font-medium text-gray-700'>
                                  Opmerkingen:
                                </h4>
                                {opmerkingen[stap.code].map(
                                  (opmerking, index) => (
                                    <div
                                      key={index}
