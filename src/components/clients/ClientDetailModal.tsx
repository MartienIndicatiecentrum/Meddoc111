...
const ClientDetailModal: React.FC<ClientDetailModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className='bg-white rounded-lg shadow-lg w-full max-w-3xl p-0 relative outline-none'
        tabIndex={-1}
      >
        {/* Sluitknop */}
        <button
          ...
          <div className='flex-1 min-w-0'>
            ...
            <div className='flex gap-2 flex-wrap mt-1'>
              <span className='badge bg-blue-100 text-blue-800 text-xs'>
                {client.care.status.replace('_', ' ')}
              </span>
              <span className='badge bg-cyan-100 text-cyan-800 text-xs'>
                {client.care.careLevel.toUpperCase()}
              </span>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'overview' && (
            <div>
              ...
              <ul className='text-sm text-gray-700'>
                <li>
                  Geboortedatum: {client.dateOfBirth.toLocaleDateString()}
                </li>
                <li>Geslacht: {client.gender}</li>
                <li>Telefoon: {client.contact.phone}</li>
                <li>Email: {client.contact.email}</li>
                <li>
                  Voorkeurscontact: {client.contact.preferredContactMethod}
                </li>
                <li>Tags: {client.tags.join(', ')}</li>
              </ul>
              <h3 className='font-semibold mt-4 mb-1'>Noodcontact</h3>
              <div className='text-xs text-gray-600'>
                {client.contact.emergencyContact.name} (
                {client.contact.emergencyContact.relationship})<br />
                {client.contact.emergencyContact.phone}
              </div>
              <div className='mt-4 text-xs text-gray-400'>
                Aangemaakt op: {client.createdAt.toLocaleDateString()} | Laatst
                bijgewerkt: {client.updatedAt.toLocaleDateString()}
              </div>
            </div>
          )}
          {activeTab === 'care' && (
            <div>
              ...
              <ul className='text-sm text-gray-700'>
                ...
                <li>
                  Startdatum zorg: {client.care.startDate.toLocaleDateString()}
                </li>
                <li>
                  Beëindigingsdatum:{' '}
                  {client.care.endDate
                    ? client.care.endDate.toLocaleDateString()
                    : '-'}
                </li>
                <li>Allergieën: {client.care.allergies?.join(', ')}</li>
                <li>
                  Medicatie: {client.care.medications?.length ? 'Ja' : 'Nee'}
                </li>
        ...
        <div className='px-6 pb-6 flex justify-end'>
          <button
            className='px-4 py-2 rounded bg-primary-600 text-white hover:bg-primary-700 focus:outline-none focus:ring'
            onClick={onClose}
          >
            Sluiten
          </button>
