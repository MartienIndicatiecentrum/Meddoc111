...
const transformDbClientToClient = (dbClient: any): Client => {
  ...
  return {
    ...
    dateOfBirth: dbClient.geboortedatum ? new Date(dbClient.geboortedatum) : new Date('1900-01-01'),
    ...
    address: {
      ...
      city: dbClient.woonplaats || parsedAddress?.city || "Geen informatie beschikbaar",
      municipality: dbClient.woonplaats || parsedAddress?.city || "Geen informatie beschikbaar",
      accessInstructions: undefined
    },
    care: {
      startDate: dbClient.created_at ? new Date(dbClient.created_at) : new Date(),
      endDate: undefined,
...
const Clienten = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const filterElement = document.querySelector('[data-filter-dropdown]') as HTMLElement;
  ...
  const filteredClients = useMemo(() => {
    return clients.filter(client => {
      ...
      if (searchTerm) {
        ...
        const searchMatch = naam.includes(searchTerm.toLowerCase()) ||
      ...
      // Status filter
      if (filters.status.length > 0 && !filters.status.includes(client.care.status)) {
        return false;
      }

      // Care level filter
      if (filters.careLevel.length > 0 && !filters.careLevel.includes(client.care.careLevel)) {
        return false;
      }

      // Assigned coordinator filter
      if (filters.assignedTo.length > 0 && !filters.assignedTo.includes(client.assignedCareCoordinator)) {
        return false;
      }

      // Client filter
      if (filters.client.length > 0 && !filters.client.includes(client.fullName)) {
        return false;
      }
  ...
  const availableClients = useMemo(() => {
    const uniqueNames = Array.from(new Set(clients.map(client => client.fullName)));
  ...
  const handleSaveClient = async (updatedClient: Client) => {
    try {
      ...
      if (selectedClientForDetails?.id === updatedClient.id) {
        setSelectedClientForDetails({ ...updatedClient, fullName, updatedAt: new Date() });
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            <Link to="/" className="flex items-center px-3 py-1.5 rounded-full border border-gray-300 hover:bg-gray-50 transition shadow-sm">
              <HomeIcon className="w-4 h-4 mr-1.5 text-gray-500" />
              <span className="text-sm text-gray-500 font-medium">Home</span>
            </Link>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-500">Cliënten</h1>
              <p className="text-gray-500 mt-1">
                Beheer en bekijk al uw cliënten ({filteredClients.length} {filteredClients.length === 1 ? 'cliënt' : 'cliënten'})
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <div className="relative" data-filter-dropdown>
              ...
              >
                ...
                {activeFiltersCount > 0 && (
                  <Badge variant="secondary" className="ml-2 text-xs bg-purple-200 text-purple-700">
                    {activeFiltersCount}
                  </Badge>
                )}
              </Button>
            ...
            <Button size="sm" className="bg-purple-100 text-purple-600 border border-purple-500 hover:bg-purple-200">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          {selectedClients.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" className="bg-white border-gray-300 text-gray-500 hover:bg-gray-50">
                Acties
              </Button>
            </div>
          )}
              ...
              <Badge key={coordinator} variant="secondary" className="flex items-center gap-1 bg-purple-100 text-purple-700 border border-purple-300">
                ...
                <X
                  ...
                  onClick={() => {
                    setFilters(prev => ({
                      ...prev,
                      assignedTo: prev.assignedTo.filter(c => c !== coordinator)
                    }));
                  }}
                />
              </Badge>
      ...
      {deletedClients.length > 0 && (
        <div className="mt-12">
          <h2 className="text-xl font-bold mb-4 text-gray-500">Verwijderde cliënten</h2>
              ...
              <tbody>
                {deletedClients.map(client => (
                  <tr key={client.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="py-2 text-gray-500">{client.contact.email}</td>
                    <td className="py-2 flex gap-2">
                      <Button size="sm" variant="outline" className="bg-white border-gray-300 text-gray-500 hover:bg-gray-50" onClick={() => handleRestoreClient(client.id)}>
                        Herstellen
                      </Button>
                      <Button size="sm" variant="destructive" className="bg-red-100 text-red-600 border border-red-500 hover:bg-red-200" onClick={() => setShowConfirmDelete(client.id)}>
                        ...
                        <Dialog open={true} onOpenChange={() => setShowConfirmDelete(null)}>
                          <div className="fixed inset-0 bg-black/30 z-40 flex items-center justify-center">
                            <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full z-50">
                              <h3 className="text-lg font-bold mb-2 text-gray-500">Weet je het zeker?</h3>
                              <p className="mb-4 text-gray-500">Deze cliënt wordt <b>permanent</b> verwijderd en kan niet meer worden hersteld.</p>
                              <div className="flex gap-2 justify-end">
                                <Button variant="outline" className="bg-white border-gray-300 text-gray-500 hover:bg-gray-50" onClick={() => setShowConfirmDelete(null)}>
                                  Annuleren
                                </Button>
                                <Button variant="destructive" className="bg-red-100 text-red-600 border border-red-500 hover:bg-red-200" onClick={() => handlePermanentDelete(client.id)}>
