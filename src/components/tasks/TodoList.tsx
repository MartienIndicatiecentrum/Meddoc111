...
const TodoList: React.FC<TodoListProps> = ({
  ...
  const getDeadlineStatus = (deadline?: string) => {
    if (!deadline) {
      return { status: 'no-deadline', color: 'text-gray-500', icon: null };
    }
      ...
      if (deadlineFilter && deadlineFilter !== 'all') {
        switch (deadlineFilter) {
          case 'overdue':
            ...
            break;
          case 'today':
            if (deadlineStatus.status !== 'today') {
              return false;
            }
            break;
          case 'soon':
            if (deadlineStatus.status !== 'soon') {
              return false;
            }
            break;
          case 'no-deadline':
  ...
  const [sortBy, setSortBy] = useState<'alphabetical' | 'urgency' | 'deadline'>(
    'alphabetical'
  );
    ...
    switch (sortBy) {
      case 'urgency': {
        return tasksToSort.sort((a, b) => {
          const aUrgency =
            urgencyOrder[a.urgentie_status as keyof typeof urgencyOrder] || 0;
          const bUrgency =
            urgencyOrder[b.urgentie_status as keyof typeof urgencyOrder] || 0;
          ...
          <Select value={clientFilter} onValueChange={setClientFilter}>
            ...
            <SelectContent>
              ...
              {Object.values(clientMap)
                .filter(
                  clientName => clientName && clientName !== 'Onbekende cliënt'
                )
                .sort()
        ...
        <div className='space-y-3'>
          {sortedTasks.length === 0 ? (
            ...
            sortedTasks.map(task => {
              const deadlineStatus = getDeadlineStatus(task.deadline);
              const clientName =
                clientMap[task.client_id] || 'Onbekende cliënt';
                      ...
                      <div className='flex-1 min-w-0'>
                        ...
                        <div className='flex flex-wrap gap-1 mb-1'>
                          ...
                          >
                            ...
                            >
                              <SelectValue>
                                {task.status === 'afgehandeld'
                                  ? 'Afgerond'
                                  : task.status === 'in_behandeling'
                                    ? 'In behandeling'
                                    : task.status === 'wachten_op_info'
                                      ? 'Wachten op info'
                                      : task.status === 'opvolging'
                                        ? 'Opvolging'
                                        : 'Nieuw'}
                              </SelectValue>
                            </SelectTrigger>
                            <SelectContent>
                              ...
                              <SelectItem value='wachten_op_info'>
                                Wachten op info
                              </SelectItem>
                              <SelectItem value='opvolging'>
                                Opvolging
                              </SelectItem>
                          ...
                          >
                            ...
                            >
                              <SelectValue>
                                {task.urgentie_status === 'urgent'
                                  ? 'Urgent'
                                  : task.urgentie_status === 'hoog'
                                    ? 'Hoog'
                                    : task.urgentie_status === 'normaal'
                                      ? 'Normaal'
                                      : 'Laag'}
                              </SelectValue>
                            </SelectTrigger>
                          ...
                          <Select
                            ...
                            onValueChange={newTaskType => {
                              if (onTaskTypeChange) {
                                onTaskTypeChange(
                                  task.id,
                                  newTaskType === 'no-type' ? '' : newTaskType
                                );
                              }
                            }}
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value='indicatie'>
                                Indicatie
                              </SelectItem>
                              <SelectItem value='behandeling'>
                                Behandeling
                              </SelectItem>
                              <SelectItem value='controle'>Controle</SelectItem>
                              <SelectItem value='administratie'>
                                Administratie
                              </SelectItem>
                              <SelectItem value='overleg'>Overleg</SelectItem>
                              <SelectItem value='documentatie'>
                                Documentatie
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        ...
                        {task.deadline && (
                          <div
                            className={`flex items-center gap-1 text-xs ${deadlineStatus.color}`}
                          >
                            {deadlineStatus.icon}
                            <span>
