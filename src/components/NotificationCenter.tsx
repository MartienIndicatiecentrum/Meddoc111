...
interface Notification {
  id: string;
  type: 'deadline_approaching' | 'status_change' | 'new_document' | 'ai_insight';
  title: string;
  message: string;
  document_id?: string;
...
export const NotificationCenter: React.FC = () => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'deadline_approaching': return <Calendar className="h-4 w-4 text-orange-500" />;
      case 'status_change': return <CheckCircle className="h-4 w-4 text-green-500" />;
      case 'new_document': return <FileText className="h-4 w-4 text-blue-500" />;
      case 'ai_insight': return <AlertTriangle className="h-4 w-4 text-purple-500" />;
          ...
          <CardContent className="p-0">
            ...
            {notifications.length > 5 && (
              <div className="p-4 border-t">
                ...
                >
                  {showAll ? 'Toon minder' : `Toon alle ${notifications.length} notificaties`}
                </Button>
              </div>
            )}
          </CardContent>
