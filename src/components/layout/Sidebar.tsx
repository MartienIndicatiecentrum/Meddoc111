...
const navItems = [
  ...
  { label: 'Documenten', icon: Folder, to: '/documenten' },
  {
    ...
    icon: Bot,
    to: 'https://anam-cara.vercel.app/',
    external: true,
  },
  { label: 'Document Upload', icon: Upload, to: '/document-upload' },
  { label: 'AI Chat (coming soon)', icon: MessageCircle, disabled: true },
  ...
  {
    label: 'Factuur Generator',
    icon: FileSpreadsheet,
    to: '/factuur-generator',
  },
...
const Sidebar: React.FC = () => {
  const { documentCount, loading: documentCountLoading } =
    useTotalDocumentCount();

  return (
    <aside className='w-64 bg-white border-r hidden md:flex flex-col'>
      <div className='h-16 flex items-center px-6 font-bold text-primary-600 text-xl'>
        <span className='mr-2'>🩺</span> MedDoc Demo
      </div>
        ...
        {navItems.map(item => {
          ...
          if (item.disabled) {
            return (
              <div
                key={item.label}
                className='flex items-center gap-3 px-3 py-2 rounded text-gray-400 bg-gray-50 cursor-not-allowed opacity-60'
                title='Deze functie is binnenkort beschikbaar'
              >
                <item.icon className='w-5 h-5' />
                <span>{item.label}</span>
              </div>
