  ...
  const [messages, setMessages] = useState([
    {
      ...
      type: 'bot',
      content:
        'Hallo! 👋 Ik ben je AI assistent voor cliëntgegevens. Ik kan je helpen met het opzoeken en analyseren van cliëntinformatie uit je Supabase database. Wat wil je weten?',
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage = {
        ...
        type: 'bot',
        content:
          '❌ Er is een onverwachte fout opgetreden. Controleer of de MCP server draait.',
        timestamp: new Date(),
        success: false,
      };
      ...
      <div className='flex-1 overflow-y-auto p-4 space-y-4'>
        {messages.map(message => (
          ...
          >
            ...
            >
              <div
                className={`p-2 rounded-full ${message.type === 'user' ? 'bg-blue-500' : 'bg-gray-600'}`}
              >
                {message.type === 'user' ? (
      ...
      <div className='bg-white border-t border-gray-200 p-4'>
        <div className='flex gap-3'>
          ...
          <button
            ...
            disabled={
              !inputMessage.trim() || isLoading || serverStatus === 'error'
            }
        ...
        <div className='mt-2 text-xs text-gray-500'>
          Voorbeeldvragen: "Dashboard overzicht", "Documenten van Arkojan",
          "Urgente documenten", "Cliënt statistieken"
        </div>
        ...
        {serverStatus === 'error' && (
          <div className='mt-2 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700 flex items-center gap-2'>
            ...
            <span>
              ⚠️ Kan geen verbinding maken met de MCP server. Zorg dat de server
