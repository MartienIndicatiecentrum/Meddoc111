  ...
  const folderCounts = React.useMemo(() => {
    ...
    folders.forEach(folder => {
      const count = documents.filter(
        doc => doc.type !== 'folder' && doc.category === folder.title
      ).length;
        ...
        <div>
          ...
          <ReactSelect
                ...
                ? clients
                    .map(client => {
                      const docCount = documents.filter(
                        doc =>
                          doc.client_id &&
                          String(doc.client_id) === String(client.id)
                      ).length;
                      return {
            ...
            options={clients.map(client => {
              const docCount = documents.filter(
                doc =>
                  doc.client_id && String(doc.client_id) === String(client.id)
              ).length;
              return {
                value: client.id.toString(),
                label: `${client.naam} (${docCount})`,
          ...
          <div className='flex flex-wrap gap-1'>
            ...
            {selectedClient && (
              <span className='bg-green-100 text-green-800 px-2 py-1 rounded text-xs'>
                Cliënt:{' '}
                {clients.find(c => c.id.toString() === selectedClient)?.naam ||
                  'Onbekend'}
              </span>
            )}
            {category && (
            ...
            {selectedFolder &&
              (() => {
                ...
                const folder = folders.find(
                  f => f.id.toString() === selectedFolder
                );
                ...
                const borderColor = folder
                  ? pastelColorFromId(folder.id)
                  : '#eee';
                const idStr = folder ? String(folder.id) : '0';
                const textColor = folder
                  ? `hsl(${idStr.split('').reduce((a, c) => a + c.charCodeAt(0), 0) % 360}, 60%, 35%)`
                  : '#555';
                return (
                  <span
                    className='px-3 py-1 rounded-full text-xs flex items-center gap-1 border'
