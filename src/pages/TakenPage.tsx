...
const fetchTasks = async (): Promise<Task[]> => {
  try {
    ...
    const mappedTasks = (data || []).map((row: SupabaseTaskRow): Task => ({
      id: row.id,
      ...
      {(provided, snapshot) => (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-lg text-gray-800">{column.label}</h3>
...
const TakenPage: React.FC = () => {
  ...
  const createTaskMutation = useMutation({
    mutationFn: async (taskData: typeof newTaskData & { status: string }) => {
      ...
      if (taskData.extra_notitie) {
        combinedNotes += (combinedNotes ? '\n\n' : '') + 'Extra notitie: ' + taskData.extra_notitie;
      }
      ...
      if (taskData.huisbezoek_datum) {
        combinedNotes += (combinedNotes ? '\n\n' : '') + 'Huisbezoekdatum: ' + taskData.huisbezoek_datum;
      }
  ...
  const handleUrgencyChange = useCallback((taskId: string, newUrgency: string) => {
  ...
  const handleTaskTypeChange = useCallback((taskId: string, newTaskType: string) => {
  ...
  const handleDuplicateTask = useCallback((task: Task) => {
    toast.info(`Dupliceer taak: ${task.taakbeschrijving} - functionaliteit komt binnenkort`);
  }, []);
  ...
  const handleUploadSuccess = useCallback((fileName: string) => {
    console.log(`Document ${fileName} uploaded for task:`, selectedTaskForUpload?.taakbeschrijving);
    toast.success(`Document ${fileName} succesvol geüpload voor taak: ${selectedTaskForUpload?.taakbeschrijving}`);
    ...
    <AppLayout>
      <div className="max-w-full mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
            ...
            <div className="flex gap-4">
              <Link to="/taken" className="group">
                <div className="bg-white rounded-lg shadow-sm p-4 text-center border cursor-pointer transition hover:shadow-md hover:bg-gray-50">
                  <div className="text-2xl font-bold text-gray-900">{selectedClientId ? tasks.filter(task => task.client_id === selectedClientId).length : tasks.length}</div>
          ...
          <div className="flex flex-wrap gap-4 items-center mb-6">
            ...
            <Autocomplete
              ...
              noOptionsText={loadingClients ? "Laden..." : "Geen cliënten gevonden"}
              clearOnBlur={false}
              selectOnFocus
            />
        ...
        <div className="mb-6 flex justify-center">
          <div className="bg-white rounded-lg shadow-sm border p-1">
            <div className="flex">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2a4 4 0 014-4h4m0 0V7m0 4l-4-4m4 4l4-4" /></svg>
                Work-flow
              </a>
              <a
          ...
          <div className="flex w-full gap-4 pb-6">
            {STATUS_COLUMNS.map((column) => (
              <Column
                key={column.key}
                column={column}
                tasks={column.key === 'urgent' ? filteredTasks.filter(task => task.urgentie_status === 'urgent') : tasksByStatus[column.key]}
                clientMap={clientMap}
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="font-medium text-blue-900 mb-2">💡 Tips voor gebruik:</h4>
          <ul className="text-sm text-blue-800 space-y-1">
            <li>• Sleep taken tussen kolommen om de status te wijzigen</li>
            <li>• Gebruik de filter om taken van een specifieke cliënt te bekijken</li>
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {isEditMode ? 'Taak bewerken' : `Nieuwe taak toevoegen - ${STATUS_COLUMNS.find(col => col.key === newTaskStatus)?.label}`}
                </h3>
                <button
                  onClick={() => {
                    setShowTaskModal(false);
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (isEditMode && editingTask) {
                    ...
                    const geldigeStatussen = ['nieuw', 'in_behandeling', 'wachten_op_info', 'opvolging', 'afgehandeld', 'geannuleerd'];
                    const statusVoorInsert = geldigeStatussen.includes(newTaskStatus) ? newTaskStatus : 'nieuw';
                    createTaskMutation.mutate({ ...newTaskData, status: statusVoorInsert });
                  }
                }}
                className="space-y-4"
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, client_id: e.target.value }))}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, beschrijving: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    rows={3}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, taak_type: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    ...
                    <option value="intakegesprek_nieuwe_indicatie">Intakegesprek nieuwe indicatie</option>
                    <option value="intakegesprek_herindicatie">Intakegesprek herindicatie</option>
                    <option value="intake_verwerken_documenten_volgen">Intake verwerken; documenten volgen</option>
                    <option value="intake_verwerken_documenten_in_map">Intake verwerken; documenten in map</option>
                    <option value="intake_verwerken_fillin_form_volgt">Intake verwerken; fillin form volgt</option>
                    ...
                    <option value="kwantificatie_of_care_moet_nog">Kwantificatie of care moet nog</option>
                    <option value="wijzigingen_nodig">Wijzigingen nodig</option>
                    <option value="documenten_geupload">Documenten geupload</option>
                    <option value="vragen_verzekeraar">Vragen verzekeraar</option>
                ...
                {(newTaskData.taak_type === 'indicatie' ||
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewTaskData(prev => ({ ...prev, indicatie_type: e.target.value }))}
                      className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="">Selecteer indicatie type</option>
                      <option value="nieuwe_indicatie">Nieuwe indicatie</option>
                      <option value="verlengen_geen_wijzigingen">Verlengen indicatie; geen wijzigingen</option>
                      <option value="verlengen_met_wijzigingen">Verlengen indicatie; met wijzigingen</option>
                      <option value="wijzigen_bestaande_indicatie">Wijzigen, bestaande indicatie</option>
                    </select>
                  </div>
                )}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, prioriteit: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, taak_datum: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, taak_tijd: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewTaskData(prev => ({ ...prev, notities: e.target.value }))}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    rows={2}
                ...
                {newTaskData.taak_type === 'wijzigingen_nodig' && (
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setNewTaskData(prev => ({ ...prev, extra_notitie: e.target.value }))}
                      className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      rows={3}
                  ...
                  <>
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setNewTaskData(prev => ({ ...prev, verzekeraar: e.target.value }))}
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setNewTaskData(prev => ({ ...prev, upload_documenten: files }));
                        }}
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                {newTaskData.taak_type === 'opvolging' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewTaskData(prev => ({ ...prev, huisbezoek_datum: e.target.value }))}
                      className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  >
                    {(createTaskMutation.isPending || editTaskMutation.isPending) ? 'Bezig...' : (isEditMode ? 'Taak Bijwerken' : 'Taak Aanmaken')}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">
                  Document uploaden voor: {selectedTaskForUpload.taakbeschrijving}
                </h3>
                <button
              ...
              <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-800">
                  ...
                  <strong>Cliënt:</strong> {clientMap[selectedTaskForUpload.client_id] || 'Onbekende cliënt'}<br/>
                  <strong>Type:</strong> {selectedTaskForUpload.taaktype || 'Onbekend'}<br/>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
              <div className="flex flex-col items-center text-center">
                <div className="mb-4 text-lg text-gray-800">Weet je zeker dat je deze taak wilt verwijderen?</div>
                <div className="flex gap-3">
                  <button onClick={cancelDelete} className="px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300 transition-colors">Annuleren</button>
                  <button onClick={confirmDelete} className="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors">Verwijderen</button>
                </div>
              </div>
            </div>
          </div>
